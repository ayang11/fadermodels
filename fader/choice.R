...
control.bb=function(model,...){
	...
	return(list(x=model$raw$x,y=model$raw$y,opp=git(model$raw$opp,max(model$raw$x)),num=sum(model$raw$y),names=c('alpha','beta'),allowspike=TRUE,...))
}
ll.bb=function(model,param=NULL,x=model$control$x,opp=model$control$opp){
	alp=param[1]; bet=param[2]
	return(lchoose(opp,x)+lgamma(alp+x)+lgamma(bet+opp-x)+lgamma(alp+bet)-lgamma(alp+bet+opp)-lgamma(alp)-lgamma(bet))
}
