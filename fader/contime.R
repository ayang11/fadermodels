...
ll.exp=function(model,param=NULL,x=model$control$x){
	return(log(c(diff(1-exp(-param*x)),exp(-param*x[length(x)]))))
}
...
ll.eg=function(model,param=NULL,x=model$control$x){
	...
	return(log(c(diff(1-(alpha/(alpha+x))^r),(alpha/(alpha+x[length(x)]))^r)))
}
mean.eg=function(model) {
	warning("This hasn't been checked")
	return(model$param$alpha/(model$param$r-1))
}
vcov.eg=function(model) {
	warning("This hasn't been checked")
	return(model$param$r*model$param$alpha^2/((model$param$r-1)^2*(model$param$r-2)))
}
...
ll.wg=function(model,param=NULL,x=model$control$x){
	...
	return(log(c(diff(1-(alpha/(alpha+x^k))^r),(alpha/(alpha+x[length(x)]^k))^r)))
}
mean.wg=function(model) {
	warning("This hasn't been checked")
	return(exp(log(model$param$alpha^(1/model$param$c))+lgamma(1+1/model$param$c)+lgamma(model$param$r-1/model$param$c)-lgamma(model$param$r)))
}
vcov.wg=function(model) {
	warning("This hasn't been checked")
	return(model$param$alpha^(2/model$param$c)/gamma(model$param$r)*(gamma(1+2/model$param$c)*gamma(model$param$r-2/model$param$c)-gamma(1+1/model$param$c)^2*gamma(model$param$r-1/model$param$c)^2/gamma(model$param$r)))
}
...
ll.gg=function(model,param=NULL,x=model$control$x){
	...
	return(log(c(diff(tmp),1-tmp[length(tmp)])))
}
mean.gg=function(model) {
	warning("This hasn't been checked")
	return(model$param$alpha*model$param$s/(model$param$r-1))
}
vcov.gg=function(model) {
	warning("This hasn't been checked")
	return(model$param$alpha^2*model$param$s*(model$param$r+model$param$s-1)/((model$param$r-1)^2*(model$param$r-2)))
}
